           ...
           THCTensor *bias)
{
  ...
  /* CUDA 11+: cusparseScsrmm 已移除，使用 cusparseSpMM (Generic API) */
  ...
  cusparseCreateDnMat(&matB, inDim, outDim, inDim,
...
#endif
      , CUSPARSE_ORDER_COL);
  cusparseCreateDnMat(&matC, batchnum, outDim, batchnum,
  ...
  void* dBuffer = NULL;
  if (bufferSize > 0) {
    THCudaCheck(cudaMalloc(&dBuffer, bufferSize));
  }
  ...
  if (dBuffer) {
    THCudaCheck(cudaFree(dBuffer));
  }
  cusparseDestroySpMat(matA);
  cusparseDestroyDnMat(matB);
  cusparseDestroyDnMat(matC);
#else
  /* CUDA 10.x: 使用 legacy cusparseScsrmm */
           ...
           accreal scale)
{
  ...
  real one = ScalarConvert<int, real>::to(1);
#if CUDART_VERSION >= 11000
  /* CUDA 11+: 使用 cusparseSpMM (Generic API) */
  /* 稀疏矩阵 A: inDim x batchnum (CSC 格式，用 CSR 描述: rowPtr=colPtrs, colInd=rowInds) */
  cusparseSpMatDescr_t matA;
  cusparseDnMatDescr_t matB, matC;
  ...
  cusparseCreateDnMat(&matB, batchnum, outDim, batchnum,
...
#endif
      , CUSPARSE_ORDER_COL);
  cusparseCreateDnMat(&matC, inDim, outDim, inDim,
